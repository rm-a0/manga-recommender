...
from manga_recommender.recommender.base import DEFAULT_RANK_CONSTANT, Candidate
...
def test_weighted_rank_fusion_scores_a_better_rank_higher() -> None:
    ...
    weighted_rank_fusion(make_query(), [first, second])
...
def test_weighted_rank_fusion_applies_the_rank_constant() -> None:
    ...
    weighted_rank_fusion(make_query(), [candidate])

    assert candidate.score == 1 / DEFAULT_RANK_CONSTANT


def test_weighted_rank_fusion_reads_the_rank_constant_from_the_query() -> None:
    candidate = _candidate(content=2)

    weighted_rank_fusion(make_query(rank_constant=8), [candidate])

    assert candidate.score == 1 / 10


def test_weighted_rank_fusion_weights_each_source() -> None:
    weak, strong = _candidate(weak_source=0), _candidate(strong_source=0)
    query = make_query(source_weights={"weak_source": 0.5, "strong_source": 2.0})
...
def test_weighted_rank_fusion_adds_the_score_of_every_source() -> None:
    ...
    query = make_query(source_weights={"content": 1.0, "tags": 1.0})
...
def test_take_top_k_orders_by_score() -> None:
    ...
    result = take_top_k(make_query(), [low, high])
...
def test_take_top_k_cuts_the_list_to_the_limit() -> None:
    ...
    result = take_top_k(make_query(limit=2), candidates)
...
def test_take_top_k_keeps_the_given_order_of_equal_scores() -> None:
    ...
    result = take_top_k(make_query(), [first, second])
